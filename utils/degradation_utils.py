...
class Degradation(object):
    ...
    def _degrade_by_type(self, clean_patch, degrade_type):
        if degrade_type == 'denoise_15':
            # denoise sigma=15
            degraded_patch, clean_patch = self._add_gaussian_noise(clean_patch, sigma=15)
        elif degrade_type == 'denoise_25':
            # denoise sigma=25
            degraded_patch, clean_patch = self._add_gaussian_noise(clean_patch, sigma=25)
        elif degrade_type == 'denoise_30':
            # denoise sigma=30
            degraded_patch, clean_patch = self._add_gaussian_noise(clean_patch, sigma=30)
        elif degrade_type == 'denoise_50':
            # denoise sigma=50
